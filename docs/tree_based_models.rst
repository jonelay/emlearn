...
- ``inline``: Generates if-else code, supports ``float``, ``int32_t``, ``int16_t``, ``int8_t``
- ``loadable``: Generates struct-based code using ``eml_gbtrees.h``, requires ``dtype='int16_t'``
...
- ``loss='exponential'`` is not supported (use ``loss='log_loss'``)
- ``HistGradientBoostingClassifier`` and ``HistGradientBoostingRegressor`` are not yet supported
- Loadable mode requires ``dtype='int16_t'``
- Maximum 127 features (``int8_t`` feature index in loadable mode)
- Maximum 30 classes for loadable mode
